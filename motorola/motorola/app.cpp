...
ImVec4 decToVec4(int color[4]) {    // konwerter wartości z RGBA na akcepltowalne przez ImVec4
...
namespace App{
    ...
    int FilterDNALetters(ImGuiInputTextCallbackData* data){     // Pozwala na wprowadzanie tylko liter określających zasady azotowe
    ...
    void RenderSequenceInputScreen() {
        ...
        if (Button("Wybierz plik", ImVec2(200, 50))) {
            ImGuiFileDialog::Instance()->OpenDialog("ChooseFileDlgKey", u8"Wybierz plik z sekwencją", ".txt", ".");
            ImGuiFileDialog::Instance()->SetFileStyle(IGFD_FileStyleByExtention, ".txt", ImVec4(0, 0.7, 0, 1));     // podświetla .txt w wyborze
        };
        PopStyleColor();
        ...
        PushStyleColor(ImGuiCol_Text, ImVec4(1, 1, 1, 1));
        SetCursorPosX(szerokosc * 0.19);
        if (Button(u8"Wprowadź", ImVec2(200, 50))) {     //guzik potwierdzenia
            screen = 1;
        };
        PopStyleColor();
    }
    ...
    void RenderGUI(){
        ...
        // Style okna
        // ImVec4 przechowuje 4 parametry, każdy ma wartość od 0-1, gdzie 1 to 255
        ...
        ImGui::SetNextWindowSize(ImVec2(main_viewport->Size.x, main_viewport->Size.y));     // Rozmiar wewnętrznego okna dopasowany do zewnętrznego
        Begin("Bioinformatyka", NULL, window_flags);    //tytuł okna, czy się zamyka, cechy
